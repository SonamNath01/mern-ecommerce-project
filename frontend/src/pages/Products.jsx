...
const Products = () => {
  // Get product ID from the URL (like /product/:productId)
  const { productId } = useParams();

  // Access global context (products list, currency, and cart handler)
  const { products, currency, addToCart } = useContext(ShopContext);

  // Local state for selected product, image, and size
    ...
    <div className="border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100">
      ...
      <div className="mt-20 max-w-6xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div className="flex flex-col gap-4 p-6 text-sm text-gray-600 leading-relaxed bg-white">
          <p>
            An e-commerce (electronic commerce) website is an online platform
            where people can buy and sell products or services over the
            internet. It allows users to browse products, add them to a cart,
            make secure payments, and get items delivered to their doorstep.
          </p>
          <p>
            ...
            digital shopping experience where customers can browse items, add
            them to a cart, and make secure payments — all from the comfort of
            their devices.
          </p>
        </div>
      </div>
